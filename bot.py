...
class Bot(Client):
    ...
    async def start(self):
        ...
        try:
            ...
            self.LOGGER(__name__).info("\nBot Stopped. @purushhelp for support")
            sys.exit()

        self.set_parse_mode(ParseMode.HTML)
        self.username = usr_bot_me.username
        self.LOGGER(__name__).info(f"Bot Running..! Made by @purushhelp")   
        ...
        try: await self.send_message(OWNER_ID, text = f"<b><blockquote>🤖 Bᴏᴛ Rᴇsᴛᴀʀᴛᴇᴅ by @purushhelp</blockquote></b>")
        except: pass
    ...
    def run(self):
        ...
        self.LOGGER(__name__).info("Bot is now running. Thanks to @purushhelp")
        try:
            loop.run_forever()
